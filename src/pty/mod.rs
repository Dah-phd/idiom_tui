...
impl PtyShell {
    ...
    pub fn map_key<B: Backend<Style = ContentStyle, Color = Color>>(
        &mut self,
        key: &KeyEvent,
        backend: &mut B,
    ) -> std::io::Result<()> {
    ...
    pub fn map_mouse<B: Backend<Style = ContentStyle, Color = Color>>(
        &mut self,
        event: MouseEvent,
        backend: &mut B,
    ) -> Message {
    ...
    pub fn fast_render<B: Backend<Style = ContentStyle, Color = Color>>(
        &mut self,
        backend: &mut B,
    ) {
    ...
    pub fn render<B: Backend<Style = ContentStyle, Color = Color>>(&mut self, backend: &mut B) {
        _ = self.parser.try_parse();
        self.inner_render(backend);
    }

    fn inner_render<B: Backend<Style = ContentStyle, Color = Color>>(&mut self, backend: &mut B) {
    ...
    fn render_no_select<B: Backend<Style = ContentStyle, Color = Color>>(
        &mut self,
        backend: &mut B,
    ) {
    ...
    fn render_with_select<B: Backend<Style = ContentStyle, Color = Color>>(
        &mut self,
                        ...
                        if start_line as usize == index {
                            for cell_col in 0..from.col {
                                if let Some(cell) = screen.cell(from.row, cell_col) {
                                    ...
                                    backend.print_styled(cell.contents(), style).clone();
                                };
                            }
                        }
